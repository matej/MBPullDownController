...
- (CGFloat)computedClosedTopOffset {
    CGFloat closedTopOffset = self.closedTopOffset;
    if (@available(iOS 11, *)) { closedTopOffset += self.view.safeAreaInsets.top; }
    return closedTopOffset;
}

- (CGFloat)computedOpenBottomOffset {
    CGFloat openBottomOffset = self.openBottomOffset;
    if (@available(iOS 11, *)) { openBottomOffset += self.view.safeAreaInsets.bottom; }
    return openBottomOffset;
}
...
- (void)setOpen:(BOOL)open animated:(BOOL)animated {
    ...
    CGFloat offset = open ? self.view.bounds.size.height - self.computedOpenBottomOffset : self.computedClosedTopOffset;
...
- (void)checkOpenCloseConstraints {
    ...
    if (!open && enabled && offset.y < - [self computedOpenDragOffset] - self.computedClosedTopOffset) {
        [self setOpen:YES animated:YES];
    } else if (open) {
        if (enabled && offset.y > [self computedCloseDragOffset] - self.view.bounds.size.height + self.computedOpenBottomOffset) {
